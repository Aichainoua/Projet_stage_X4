...
// URL de ton API (Ajuste si nécessaire)
    ...
    <div v-else class="overflow-hidden bg-white border border-gray-200 rounded-lg shadow-sm">
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-sm divide-y divide-gray-100">
          ...
          <tr class="transition hover:bg-gray-50" v-for="formation in formations" :key="formation.id">
            ...
            <td class="px-6 py-4">
                <span class="px-2 py-1 text-xs font-bold text-indigo-700 uppercase rounded bg-indigo-50">
                    {{ formation.niveau }}
                </span>
            </td>
            <td class="px-6 py-4">
                {{ formation.cours_count || 0 }}
            </td>
            <td class="px-6 py-4 text-center">
              <NuxtLink :to="`/apprenant/formation/${formation.id}`" class="inline-flex items-center justify-center p-2 text-indigo-900 transition rounded-full hover:bg-indigo-50">
